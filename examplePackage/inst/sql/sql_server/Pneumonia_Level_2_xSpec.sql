...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (36213519,36220586,36220577)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (36213519,36220586,36220577)
  and c.invalid_reason is null
...
AND C.condition_type_concept_id  in (45756835,44786627,38000250,38000199,38000183,38000268,38000230,38000200,38000248,38000184,45756843,44786628,38000215)
...
-- End Condition Occurrence Criteria
  ...
  WHERE DATEADD(day,0,OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,5,E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
) P
WHERE P.ordinal = 1
-- End Primary Events
