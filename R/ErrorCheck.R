errorCheck <- function(callingProgram,
  ...
  if(callingProgram == "createPhenotypeModel" | callingProgram == "createEvaluationCohort" | callingProgram == "testPhenosFromFile") {

    if(!file.exists(file.path(outFolder))) {
      stop(paste0("Error: ", file.path(outFolder, " does not exist. \n Select an OutFolder that exists")))
    }
  ...
  if(callingProgram == "testPhenosFromFile") {
    ...
    for(i in 1:nrow(cohortDefinitionsToTest)){
      if(!(is.numeric(cohortDefinitionsToTest$atlasId[[i]])) | is.na(cohortDefinitionsToTest$atlasId[[i]]) | !(is.numeric(cohortDefinitionsToTest$washoutPeriod[[i]]))  | is.na(cohortDefinitionsToTest$washoutPeriod[[i]])) {
...
checkCohortCounts <- function(connectionDetails,
                              sql,
                              action) {
