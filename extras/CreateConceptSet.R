...
sql <- SqlRender::renderSql(sqlScript,
                            ...
                            x_spec_cohort = x_sens_cohort, #x_spec_cohort,
                            tempDB = "scratch.dbo",
                            ...
                            startDate = "20100101",
                            endDate = "21000101",
                            baseSampleSize = 1.5e+06,
                            xSpecSampleSize = 0.5e+06,
                            noise = 0,
                            mainPopnCohort = 0,
...
conditionList <- data.frame(plpData$covariateRef[plpData$covariateRef$analysisId == 102,])$covariateId
...
plpData <- PatientLevelPrediction::getPlpData(connectionDetails,
                                              cdmDatabaseSchema = paste(cdm_database_schema,
                                                                        sep = ""),
                                              cohortId = 0,
                                              outcomeIds = x_sens_cohort, #x_spec_cohort,
...
population <- PatientLevelPrediction::createStudyPopulation(plpData,
                                                            population = NULL,
                                                            outcomeId = x_sens_cohort, #x_spec_cohort,
...
#plpData <- loadPlpData(file.path(workFolder, "plpData_TESTeval_10XJIDNar_ccae217_ePPV0.75_20190201v1"))
#appResults <- readRDS(file.path(workFolder, "lr_results_TESTeval_10XJIDNar_ccae217_ePPV0.75_20190201v1.rds"))
...
#covSummary <- dplyr::summarize(group_by(covDataAll, as.character(covariateId)), n(), sum(value), mean(value))
covSummary <- as.data.frame(covDataAll[, j=list(mean(value, na.rm = TRUE), sum(value, na.rm = TRUE)), by = covariateId])
...
covSummary102 <- covSummary[covSummary$analysisId == 102 & covSummary$n >= 10 & covSummary$mean >= 0.2,]
covSummary102 <- covSummary102[with(covSummary102, order(-mean)), ]

{
  tp <- 0; fp <- 0; fn <- 0; prevTp <- 0; prevFp <- 0; prevFn <- 0; removeCnt <- 0;
  prevListUp <- c(); listUp <- c(); covListUp <- c(); prevCovListUp <- c();
  for(covUp in 1:nrow(covSummary102)) {
    ...
    writeLines(paste(covUp, "\ttp: ", round(tp, 0), "\tfn:", round(fn,0), "\tsens: ", round(sens, 3), "\tfp: ",
                     round(fp,0), "\t(", round(covSummary102$mean[covUp],2), ")",covName, sep=""))
    priorList <- listUp
    ...
    if(((tp-prevTp) < 1 | ((tp-prevTp) <= (fp-prevFp)/100))) {
        ...
        writeLines(paste(covUp-1, "\ttp: ", round(tp, 0), "\tfn:", round(fn,0), "\tsens: ", round(sens, 3), "\tfp: ", round(fp,0), "\n"))
