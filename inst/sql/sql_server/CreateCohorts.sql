...
from (select 0 as COHORT_DEFINITION_ID, person_id as SUBJECT_ID, dateadd(day, 0, visit_start_date) COHORT_START_DATE, dateadd(day, 1, visit_start_date) COHORT_END_DATE
      from (select 
				{@mainPopnCohort == 0} ? {
					...
					where substring(cast(v.person_id as varchar), 5, 1) in ('5')
						{@exclCohort != 0} ? { and v.person_id not in (
													select subject_id
													from @cdm_database_schema.ohdsi_results.cohort
				...
				{@mainPopnCohort != 0} ? {
					...
					where co.cohort_definition_id = @mainPopnCohort
						and substring(cast(co.subject_id as varchar), 5, 1) in ('5')
						{@exclCohort != 0} ? { and v.person_id not in (
													select subject_id
													from @cdm_database_schema.ohdsi_results.cohort
													where COHORT_DEFINITION_ID = @exclCohort)}
	  ...
	  union
	  select 0 as COHORT_DEFINITION_ID, SUBJECT_ID, dateadd(day, 0, COHORT_START_DATE) COHORT_START_DATE, dateadd(day, 1, COHORT_START_DATE) COHORT_END_DATE
      from (select top (@xSpecSampleSize/10) * from @cdm_database_schema.ohdsi_results.cohort
			where COHORT_DEFINITION_ID = @exclCohort and substring(cast(subject_id as varchar), 5, 1) in ('5')) noise -- add a little noise to the base cohort
      ) allSub
;
