...
The system uses the prevalence cohort to provide a reasonable approximation of the prevalence of the HOI in the population. This improves the calibration of the predictive model.  This group of subjects will be used in the model building process and is described in detail below.  An [**example**](http://www.ohdsi.org/web/atlas/#/cohortdefinition/1770119) of an prevalence cohort for T2DM is also in the OHDSI ATLAS repository.

PheValuator uses the evaluation cohort to define a specific set of subjects to test for the outcome of interest and uses these subjects to evaluate the PAs.  An [**example**](http://www.ohdsi.org/web/atlas/#/cohortdefinition/1778258) of an evaluation cohort for T2DM is also in the OHDSI ATLAS repository.
...
1) An example of a prevalent algorithm for T2DM [**(Type 2 Diabetes Mellitus (prevalent))**](http://www.ohdsi.org/web/atlas/#/cohortdefinition/1778259)
2) An example of a prevalent algorithm for T2DM requiring a second condition code for T2DM 31-365 days after index [**(Type 2 diabetes mellitus (prevalent) with second code 31-365 days after index)**](http://www.ohdsi.org/web/atlas/#/cohortdefinition/1778260)
...
* baseSampleSize - set the size of the evaluation cohort (default = 2M)
* lowerAgeLimit - set the lower age limit (default = 0)
* upperAgeLimit - set the upper age limit (default = 120)
* gender - set the gender (default = all genders)
* race - set the races of the subjects to be evaluated (default = all races)
* ethnicity - set the ethnicity of the subjects to be evaluated (default = all ethnicities)
* startDate - set the start date for the analysis (default = "19001010")
* endDate- set the end date for the analysis (default = "21000101")
...
![PheValuator results for Type 2 Diabetes Mellitus analysis](D:/git/PheValuator/vignettes/pheValuatorT2DMResults.png)
